...
@router.get(path="/user/total-owed-by")
async def get_total_reimbursements_owed_by_user(current_user: TokenData = Depends(get_current_user)) -> float:
    ...
    """
    return await fetch_total_reimbursements_owed_by_user(str(current_user.id))


@router.get(path="/user/total-owed-to")
async def get_total_reimbursements_owed_to_user(current_user: TokenData = Depends(get_current_user)) -> float:
    ...
    """
    return await fetch_total_reimbursements_owed_to_user(str(current_user.id))


@router.get(path="/user/{user_id}/total-owed-by")
async def get_total_reimbursements_owed_by_specific_user(
